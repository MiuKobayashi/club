...
- ヘッダーの固定<br>
    - 画面遷移を容易に
- 登録画面のセレクトボックスの連動<br>
    - 曲を選択したらその曲のパートのみが選択できるようにする、スタート時間を選択したらそのあとの時間のみが選択できるようにする、といった仕様に
...
- 終日イベントと時間指定イベントの表示切替<br>
    - お稽古を行う日と個人のお稽古時間の表示を分ける
- 「自分の予定」「部員の予定」ボタン<br>
    - デフォルトでは自分のお稽古のみを表示、前後の人の予定も見ることができる
- イベントの色分け<br>
    - 「お稽古」「合奏練習」「その他」でカラーを分類

### APIを用いた管理者の機能
- YouTube DATA APIを使用し、以下の機能を実装した。
    - 動画検索機能
        - 管理者の曲目登録画面にて、曲名を入力すると、「曲名 + 箏」で検索される
        - 検索結果上位3件が表示され、その画面のまま再生できる
        - URLをコピーし、曲目登録の際にURLもデータベースに登録できる
        - 検索ボックスで検索した曲名は、曲名登録の入力フォームにそのまま表示される<br><br>
![動画検索画面](public/images/search.png "search")
    - 動画一覧表示機能
        - 部員はURLが登録された曲の動画一覧画面を閲覧し、その画面のまま再生できる<br><br>
![動画一覧画面](public/images/movie.png "movie")

- ChatGPT APIを使用し、「お稽古順案作成機能」を実装した。
    - 来月のお稽古の日にちボタンをクリックすると、提出された部員の希望時間をもとに文章が作成される
    - 作成に30秒程度かかるため、ローディングアイコンを表示<br><br>
![お稽古順案作成画面](public/images/chatgpt1.png "plan")
    - それに対するChatGPTの応答が画面に表示される<br><br>
![お稽古順案表示画面](public/images/chatgpt2.png "plan")
![お稽古順案作成画面](public/images/chatgpt3.png "plan")
