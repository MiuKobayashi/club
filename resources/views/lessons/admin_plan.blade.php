<x-app-layout>
    <div class="m-5">
        ...
        <div class="mt-10">
            <form method="POST" action="{{ route('planCreate') }}" class="w-full flex justify-center">
                ...
                @foreach($attendances as $attendance)
                    <button name="sentence" value="{{ $attendance->id }}" class="shadow-lg bg-pink-900 shadow-pink-900/50 hover:bg-pink-700 text-white text-2xl rounded px-2 py-1 m-1 w-1/5 ">{{ $attendance->start_date->format('m/d') }}</button>
                @endforeach
            </form>
        </div>
        ...
        <div class="flex justify-center">
            <div class="mt-10 bg-white border-2 border-pink-900 radius p-10 w-1/2">
